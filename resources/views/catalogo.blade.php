    ...
    <div class="py-12 bg-gray-100 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white p-4 rounded-lg flex flex-wrap justify-center gap-4 mb-6 shadow-sm border border-gray-200">
                <a href="{{ route('catalogo') }}" class="bg-blue-600 text-white px-6 py-2 rounded-md shadow-md font-bold">Catálogo</a>
            ...
            <div class="mb-8 flex justify-center">
                <form action="{{ route('catalogo') }}" method="GET" class="w-full md:w-1/2 flex shadow-md rounded-md overflow-hidden">
                    <input type="text" name="buscar" value="{{ request('buscar') }}" placeholder="Buscar por nombre o código QR..." 
                           class="w-full border-gray-300 bg-white text-gray-900 placeholder-gray-400 p-4 text-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2 font-black transition uppercase tracking-widest">
                        Buscar
                    </button>
                </form>
            </div>
            ...
            <div class="grid grid-cols-1 gap-8">
                    ...
                    <div class="bg-white overflow-hidden shadow-xl sm:rounded-3xl p-8 border-t-8 {{ $item->estado_actual == 'Disponible' ? 'border-green-500' : 'border-red-500' }} border-x border-b border-gray-200">
                        <div class="flex flex-col md:flex-row items-start md:items-center">
                            ...
                            <div class="flex flex-col items-center space-y-4 md:mr-12 mb-6 md:mb-0 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                                ...
                                <button onclick="downloadQR('qr-{{ $item->id_activo }}', '{{ $item->codigo_qr }}')" class="text-sm bg-white hover:bg-gray-100 text-blue-600 px-5 py-2 rounded-full border border-blue-200 shadow-sm transition flex items-center font-black uppercase tracking-tighter">
                                    💾 Descargar QR
                                </button>
                            </div>
                            ...
                            <div class="flex-grow space-y-2">
                                ...
                                <div class="flex items-center space-x-3">
                                    <span class="text-sm font-mono bg-blue-50 text-blue-700 px-3 py-1 rounded-md font-bold border border-blue-100 italic">ID: {{ $item->codigo_qr }}</span>
                                    ...
                                    <form action="{{ route('prestamos.store') }}" method="POST" class="space-y-4">
                                        ...
                                        <div>
                                            ...
                                            <input type="text" name="nombre_solicitante" required placeholder="Nombre completo" 
                                                   class="block w-full rounded-lg border-gray-300 bg-white text-gray-900 p-3 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm">
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <textarea name="condiciones_entrega" required rows="2" placeholder="Describa el estado físico actual..." 
                                                      class="block w-full rounded-lg border-gray-300 bg-white text-gray-900 p-3 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm text-sm"></textarea>
                                        </div>
                                        ...
                                        <div class="border-t border-gray-200 pt-3">
                                            ...
                                            <input type="datetime-local" name="fecha_devolucion_prevista" required 
                                                ...
                                                class="block w-full rounded-lg border-gray-300 bg-white text-gray-900 p-3 mt-1 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 shadow-sm text-sm">
                                            @error('fecha_devolucion_prevista')
                                                <span class="text-red-500 text-xs font-bold mt-1 block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        
                                        <button type="submit" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black py-4 rounded-xl shadow-lg transition transform active:scale-95 text-lg uppercase tracking-widest">
                                            Préstamo
                                        </button>
                                    </form>
                                @else
                                    <div class="text-center py-10">
                                        ...
                                        <p class="text-xs text-gray-400 mt-4 italic tracking-tighter">Consulte el panel de préstamos para detalles del retorno.</p>
                                    </div>
                                @endif
                            </div>
                        </div>
                ...
                @empty
                    <div class="text-center py-20 bg-white rounded-3xl shadow-inner border-2 border-dashed border-gray-200">
                        <p class="text-gray-400 text-2xl font-medium">No se encontraron resultados para tu búsqueda.</p>
                        <a href="{{ route('catalogo') }}" class="text-blue-500 font-bold underline mt-4 inline-block">Ver todos los instrumentos</a>
                    </div>
                @endforelse
            </div>
        </div>
